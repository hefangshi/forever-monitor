...
Monitor.prototype.start = function (restart, count, index) {
  ...
  var newChildren = self.isSpawn ? this.trySpawn() : this.tryFork(count || self.forkCount);
  ...
  function onChildExit(code, signal) {
    var child = this;
    var pid = child.pid;
    var spinning = Date.now() - self.ctime[pid] < self.minUptime;
  ...
  newChildren.forEach(function (child, newIndex) {
    ...
    process.nextTick(function () {
      self.emit(restart ? 'worker:restart' : 'worker:start', child.pid, child.index);
    });
  });
...
Monitor.prototype.kill = function (forceStop) {
  ...
  if (!childrenCopy || !childrenCopy.length || (!this.running && !this.forceRestart)) {
    ...
    childrenCopy.forEach(function (child) {
      ...
      if (forceStop) {
        ...
        if (this.killTTL) {
          var timer = setTimeout(function () {
            common.kill(child.pid, self.killTree, self.killSignal || 'SIGKILL');
          }, this.killTTL);
      ...
      common.kill(child.pid, this.killTree, this.killSignal);
    });
  }
  childrenCopy = null;
  return this;
